...
Shape::Shape(Eigen::Vector2f vel):vel(vel){}
...
bool Shape::contains(float x, float y){
	...
	for (int i=0, j=len-1; i<len; j=i++){
		Eigen::Vector2f& vi = vertices[i], &vj = vertices[j];
...
Shape* Shape::generateSnowball(Eigen::Vector2f origin, float radius, Eigen::Vector2f vel) {
