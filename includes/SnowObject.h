...
// Source: https://github.com/kodai100/OpenGL_MPMSnowSimulation2D, https://github.com/Azmisov/snow
...
public:
	...
	//Credit: https://github.com/kodai100/OpenGL_MPMSnowSimulation2D
	static SnowObject* createShape(std::vector<Shape*>& snow_shapes){
		...
		SnowObject*obj = new SnowObject(particles);
		...
		for (int i=0; i<len; i++){
			...
			Eigen::Vector2f vel = snow_shapes[i]->vel;

			if (vel.squaredNorm() > max_vel) max_vel = vel.squaredNorm();
			...
			if (a > AREA_EPSILON && snow_shapes[i]->vertices.size() > 2){
				...
				while (points_found != points){
					...
					if (snow_shapes[i]->contains(tx, ty)){
						...
						obj->particles.push_back(Particle(Eigen::Vector2f(tx, ty), vel, particle_mass));
